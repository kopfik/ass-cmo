...
- [x] `feature` Added an "Also delete the inventory record for this host" checkbox (default checked) to the Revoke agents view. It deletes the corresponding `inventory` row in the same transaction as the `agent_auth` revoke. The `agent_auth` row and its audit fields (`revoked_at`, `revoked_by`, `revoked_reason`) are never deleted by this action, regardless of the checkbox; only the visible inventory snapshot is removed.
- [x] `feature` Added a "Delete from inventory" action to already-revoked cards in the Revoke agents view (for hosts revoked before the checkbox above existed, or revoked without it checked), plus a new "Revoked agents" sidebar view listing every revoked host that still has an inventory row, shown in full without the Revoke agents 3-character search gate. Both only ever delete the `inventory` row for a UID that is confirmed `agent_auth.status = 'revoked'`; the `agent_auth` audit row is never touched.
- [ ] `later` No inactivity-based auto-revoke exists, and none is currently planned. The red-cross "offline" marker some operators see is purely a display convenience hardcoded per dashboard SQL view (e.g. `inventory_update_time < now() - interval '48 hours'` in `config.example/dashboard-views/*.sql`); it has no backend effect and does not touch `agent_auth`. If an inactivity-based auto-revoke is ever considered, it must default to a long, clearly documented threshold (30+ days, well past normal maintenance windows) and be operator-configurable rather than hardcoded, since teams may reasonably disagree with any fixed default.
- [ ] `later` Brainstorm only, not decided, pending team feedback: operators who run one ass-cmo instance per company/context (work A, work B, home) face dashboard sprawl (separate URL/login per instance, no combined view). Do not solve this by making the server multi-tenant: SECURITY.md already commits to a single-operator, self-hosted trust model without tenant isolation, and adding tenant-isolation logic to the server would introduce a classic multi-tenant SaaS bug class (cross-tenant data leak from an application-logic isolation mistake) for a use case (local admin convenience) that doesn't need it. Instead, consider a client-side aggregator that holds credentials for N independent instances and merges their (read-only, at least initially) views locally; each instance stays fully isolated since nothing but the client ever crosses between them. This needs a token-authenticated read API on each instance first, since the current dashboard is server-rendered HTML behind session cookie + CSRF, not built for cross-origin programmatic access. Prefer a bearer-token API + a static client-side SPA (no backend, optionally installed as a PWA like the existing dashboard) over an Electron-style native app or a Docker-packaged local web app: a pure static SPA needs the least new code, and works on Windows out of the box (no WSL/Docker Desktop prerequisite), since it only needs a browser making direct authenticated requests to each instance's API.
- [ ] `later` Related to the aggregator idea above: one agent reporting to multiple ass-cmo servers ("multihoming") is not blocked today — nothing stops running two independent agent installs (two configs, two systemd timers/cron entries) on the same host, each with its own per-server secret from its own enrollment. This is intentional and should stay this way: a secret should keep meaning "one host, one server", not be shared across servers, so revoking access to one company's server can never silently also affect another's, and compromising one credential never has a multi-server blast radius. Any future UX work here should package "run this agent config against a second server" more conveniently, not merge secrets across servers.
...
- [x] `feature` Grouped/collapsible dashboard tables, opted into per view with a `-- group-by: <column>` header. Implemented as client-side grouping in `app/public/assets/dashboard.js`, so it works for any view, not just the Docker ones. The rejected alternative was a dedicated PHP branch in `index.php` alongside `$isEnrollment` / `$isAgentAuth` / `$isRevokedAgents`: much more code, and it would only ever serve one view.
