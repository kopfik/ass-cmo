...
- The Revoke agents view offers an optional "Also delete the inventory record for this host" action alongside revoke. This only deletes the `inventory` row (hostname, IPs, last reported data); it never deletes the `agent_auth` row or its audit fields (`revoked_at`, `revoked_by`, `revoked_reason`), and it has no effect on the fail-closed rejection above. It is purely a dashboard cleanup convenience, requires the same admin UI access, CSRF token, and explicit confirmation checkbox as revoke itself, and defaults to on but can be unchecked to keep the previous revoke-only behavior.
- The same inventory-record deletion is also reachable as a standalone "Delete from inventory" action, both on already-revoked cards in the Revoke agents view and in the dedicated "Revoked agents" view (`?view=revoked-agents`), for hosts that were revoked without the checkbox above. Both entry points share the same server-side guard: the delete only executes when the submitted UID already has a `agent_auth` row with `status = 'revoked'`, so this action cannot be used to remove inventory data for an active or disabled host. It requires the same admin UI access, CSRF token, and explicit confirmation checkbox as every other mutable dashboard action here.
