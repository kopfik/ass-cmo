...
- Linux agents now report the host's Docker containers in the inventory payload as `docker_containers`, and the server stores them in a new `docker_containers` table. Collected per container: ID, name, image reference and image ID, compose project/service/working directory, state, health status, exit code, restart count and restart policy, creation/start/stop timestamps, published ports, and bind/volume mounts. Container labels are deliberately not stored: only the four useful ones (`com.docker.compose.project`, `com.docker.compose.service`, `com.docker.compose.project.working_dir`, `org.opencontainers.image.version`) are extracted into columns, because the rest is churn (config hashes, container numbers) and label values are a common place for internal hostnames, routing rules and scheduled shell commands. `State.Health.Log` is not collected either, for the same reason and because it is unbounded in size.
- New `docker_containers` table (`database/init/005_docker_containers.sql`), keyed by `(uid, container_id)` with a foreign key to `inventory (uid)` and `ON DELETE CASCADE`, so deleting a host's inventory record — including from the revoke flow — also removes its container rows. It holds current state only and is not a history table.
- Two new example dashboard views. `70-docker-overview.sql` lists containers currently present on reporting hosts, grouped by host and compose project, with unhealthy, restarting and dead containers sorted to the top. `80-docker-cleanup.sql` lists containers that are stopped, were replaced by a recreate, or disappeared entirely, classifying each so a compose recreate leftover is distinguishable from a container that was really removed.
- The read-only dashboard role is granted `SELECT` on `docker_containers` in `database/scripts/002_dashboard_readonly_user.sql`. Applying a schema file that adds a table does not grant access to that role automatically, so the script has to be re-run after the schema on an existing instance; `TROUBLESHOOTING.md` now says so.
...
- Already-revoked cards in the Revoke agents view that still have a stored inventory record now show a "Delete from inventory" action, so a host revoked without the checkbox above can still have its inventory row cleaned up afterwards.
- Added a new "Revoked agents" sidebar view (`?view=revoked-agents`) listing every revoked `agent_auth` row that still has a matching inventory record, each with its own "Delete from inventory" action. Unlike Revoke agents, this list is shown in full immediately, without the 3-character search gate, since it's a bounded cleanup queue rather than a full search-first host list. Deleting only ever removes the `inventory` row; the underlying `agent_auth` revoke record and its audit fields are never touched by this action.

### Changed
- Linux agent bundle version is `0.9.0`. The agent collects Docker data with `docker ps -aq --no-trunc` followed by `docker inspect`, both under a `timeout`, because the agent also runs from the apt and pacman package hooks and an unresponsive Docker daemon would otherwise stall a package transaction.
- The inventory ingest now runs the inventory upsert and the Docker container sync inside a single transaction, so one scan produces one consistent state.
- A payload without a `docker_containers` key leaves stored container rows untouched. This is what an agent older than this release sends, and also what the new agent sends when Docker is installed but unreachable, so neither case wipes a host's container data. An empty `docker_containers` array is a positive statement that the host has no containers and does mark the stored rows as missing.
- Containers missing from a host's scans for more than 90 days are deleted during that host's next scan. Recreating a compose stack leaves one missing row per container behind, and the stack has no scheduler, so the purge rides along with the scan that already touches those rows.
- Windows agents do not report Docker containers yet. The payload contract is OS-agnostic, so this is an agent-side gap, not a protocol one.
...
- The custom header logo (`config.local/branding/logo/header.*`) is now scaled by CSS to fit the brand box (max 212x36 px, aspect ratio preserved). Previously a large logo rendered at its natural size and overflowed the header. The branding README documents the recommended logo dimensions.
...
### Fixed
- The documented Linux URI-handler install command in `INSTALL.md` and in the installer's final output now includes the required `ASSCMO_HANDLER_BASE_URL=...` variable, matching the command the dashboard generates. The previous short `curl ... | sh` form failed when piped because the handler installer cannot detect the server URL on the admin workstation.
- The installer no longer silently overwrites an existing `config.local/.env` whose `ASSCMO_INSTANCE_NAME` is empty or still the example placeholder. It now warns and asks an explicit confirmation with No as the default answer; when declined, the existing file is kept, only missing/weak values are filled, and the instance hostname is asked again.
- Installer `.env` writes now also escape the `|` character, so a value containing it can no longer corrupt the generated `sed` expression.
...
### Changed
- Hardened Windows agent (`agents/windows`, bumped to `0.8.1`) reboot detection: `Test-RebootRequired` now reports a pending `PendingFileRenameOperations` reboot only when the registry value exists and contains at least one non-empty string entry. A missing value, a null value, or only empty strings no longer counts as reboot-required, while real pending renames (e.g. Microsoft EdgeUpdate paths) still do. Windows Update `RebootRequired` and CBS `RebootPending` detection are unchanged.
